...
from transformers import BertEmbeddings, BertLayer, BertPooler


# 修改后的 CLIPVisionEmbeddings，使用 ViT 和 ResNet
class CLIPVisionEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        # 使用 ViT 的 patch embedding 提取细粒度特征
        self.vit_patch_embedding = nn.Conv2d(
            in_channels=3, out_channels=self.embed_dim, kernel_size=config.vit_patch_size, stride=config.vit_patch_size,
            bias=False
        )

        # 使用 ResNet 的全局平均池化提取粗粒度特征
        self.resnet_global_embedding = nn.AdaptiveAvgPool2d((1, 1))

    def forward(self, pixel_values):
        # ViT patch embedding for fine-grained features
        vit_embeds = self.vit_patch_embedding(pixel_values)
        vit_embeds = vit_embeds.flatten(2).transpose(1, 2)  # (batch_size, num_patches, embed_dim)

        # ResNet embedding for global features
        resnet_embeds = self.resnet_global_embedding(pixel_values)
        resnet_embeds = resnet_embeds.flatten(1)  # (batch_size, embed_dim)

        return vit_embeds, resnet_embeds


# 新增的 QKVAttention 类，使用文本作为 Q，粗粒度特征作为 K，细粒度特征作为 V
class QKVAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        attn_scores = torch.matmul(q, k.transpose(-1, -2))  # (batch_size, num_heads, seq_len_q, seq_len_k)
        attn_probs = nn.functional.softmax(attn_scores, dim=-1)

        # Compute attention output
        attn_output = torch.matmul(attn_probs, v)  # (batch_size, num_heads, seq_len_q, head_dim)
        attn_output = self.out_proj(attn_output)

        return attn_output


# 修改后的 UnimoEncoder，使用新增的 QKVAttention 进行跨模态交互
class UnimoEncoder(nn.Module):
    def __init__(self, vision_config, text_config):
        ...
        self.text_layers = nn.ModuleList([BertLayer(text_config) for _ in range(text_config.num_hidden_layers)])
        self.qkv_attention_layers = nn.ModuleList(
            [QKVAttention(text_config) for _ in range(text_config.num_hidden_layers)])
    ...
    def forward(
        ...
        vit_embeds, resnet_embeds = vision_embeds  # 拆分为细粒度和粗粒度特征
        text_hidden_states = text_embeds

        for idx, layer_module in enumerate(self.text_layers):
            # 文本自注意力
            text_layer_output = layer_module(
                text_hidden_states,
            ...
            # 使用 QKVAttention 进行跨模态交互，其中 Q 为文本，K 为粗粒度，V 为细粒度
            qkv_attention = self.qkv_attention_layers[idx]
            attention_output = qkv_attention(q=text_hidden_states, k=resnet_embeds.unsqueeze(1), v=vit_embeds)

            # 更新隐藏状态
            text_hidden_states = text_hidden_states + attention_output  # 残差连接
...
# 修改后的 UnimoModel，将视觉特征拆分为细粒度（ViT）和粗粒度（ResNet）特征
class UnimoModel(nn.Module):
    ...
    def forward(
        ...
        # 提取视觉特征
        vit_embeds, resnet_embeds = self.vision_embeddings(pixel_values)
        vision_embedding_output = (vit_embeds, resnet_embeds)
